export const USER_CHANGELOG = [
  {
    date: '04 Aug 2026',
    entries: [
      { icon: '💾', topic: 'Quiz Progress No Longer Lost on Accidental Close', text: "Closing the app mid-quiz (swiped away, browser killed) used to lose that entire session — nothing was saved until you properly finished or hit Quit. Every answer is now saved locally right away, and synced to the cloud every 5 questions plus a best-effort save the moment the app is closed or backgrounded, so at most the last couple of answers could ever be missed instead of the whole session." },
...
export const TECH_CHANGELOG = [
  {
    date: '04 Aug 2026',
    entries: [
      'App.jsx crash-safety: Test.jsx/VoiceTest.jsx gained an onProgress(results) callback fired after every answered question (not just at quiz end) — wired to a new handleProgress() in App.jsx. On every call: (1) always writes the full in-progress results to localStorage (wm_pending_<username>) and appends the newly-answered row(s) to the local attempt log (saveAttemptLogs) — cheap, no network; (2) flushes a batch to Supabase attempt_logs via logRemoteAttempts every 5 answers (tracked via remoteSyncedIdxRef, a running "already-synced" watermark, not a hard reset each call). All rows in one session share a single sessionTsRef timestamp (set on the first answer) across every flush, so Admin\'s Sessions view still groups them as one session despite arriving in separate batches.',
      'Deliberately did NOT move score/streak/coin updates into handleProgress — those still commit exactly once, at the existing end-of-session checkpoints (persistComplete/persistPartial), unchanged. This preserves the Review screen\'s "I spoke correctly" self-correction (which re-derives scores from the final, possibly-edited testResults array) with zero regression risk. Trade-off, called out explicitly: a self-corrected item\'s *attempt_logs* row still reflects the original (pre-correction) answer, since that row was already flushed mid-quiz before the correction happened on the Review screen — only the score/mastery state reflects the correction, not the historical log row.',
      'persistComplete/persistPartial no longer re-log the full results array (was: 100% duplicate rows on top of whatever handleProgress already flushed) — replaced with flushRemaining(results), which sends only the tail past remoteSyncedIdxRef. clearPendingSession() (removes the localStorage snapshot, resets the tracking refs) runs at the end of both.',
      'Best-effort save on accidental close: a document-level "visibilitychange" listener (not "beforeunload", which doesn\'t fire reliably on mobile/PWA close) calls flushRemaining(latestResultsRef.current) when the tab becomes hidden. Combined with the every-5 periodic flush, this means at most ~4 trailing answers can ever be lost instead of an entire session — verified live: force-closed a Playwright browser mid-quiz (no Quit, no unmount) after 7 answered questions on a real Supabase project and confirmed all 7 rows landed (5 from the periodic flush, 2 from the simulated visibilitychange flush, both sharing one session ts); separately verified a normal 3-question quiz -> Review (2 wrong) -> Continue flow produces exactly 3 rows with no duplicates and scores update as before.',
